...
{
    std::string filename = "../HW4/" + poly.name() + "_" + name + ".csv";
...
int main()
{
    ...
    std::vector<int> N_values = {2, 4, 5, 6, 7, 8, 9, 10, 12, 14};

    std::vector<double> legendre_res = create_grid(-1.0, 1.0, 800); // [-1, 1]
    std::vector<double> hermite_res = create_grid(-2.0, 2.0, 800);  // [-2,2]

    // 实例化多项式对象
    Legendre<200> legendre_poly(Method::USE_EIGEN);
    Hermite<200> hermite_poly(Method::USE_EIGEN);

    run_test("../HW4/Legendre_errors.csv", legendre_poly, legendre_res, functions, N_values);
    run_test("../HW4/Hermite_errors.csv", hermite_poly, hermite_res, functions, N_values);
